...
# ==========================================
BIN_WIDTH = 0.03 
#FILENAME = 'f2_residuals_data.txt'
FILENAME = 'f2_residuals_data_100.txt'
...
for idx, label_nosym, label_sym in PLOTS_TO_GENERATE:
    ...
    # No Sym
    if idx==0 : 
        filename_plot="Fig4a.pdf"
    else:
        filename_plot=f"dataset_{idx+1}_nosym.pdf"
    plot_single_frame(
        x, [df['res_f2_nosym']], 
        [l_map['no']], [c_map['no']], 
        ...
        save_filename=filename_plot,
        #save_filename=f"dataset_{idx+1}_nosym.pdf"
    )

    # No Sym
    if idx==0 : 
        filename_plot="Fig4d.pdf"
    else:
        filename_plot=f"dataset_{idx+1}_nosym.pdf"
    ...
    plot_single_frame(
        ...
        save_filename=filename_plot,
       #s save_filename=f"dataset_{idx+1}_sym.pdf"
    )
