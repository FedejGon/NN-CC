...
#    plt.show()
    ...
    delta_ident_param, alpha_ident_param, beta_ident_param , bias_ident_param = params
    print(f"Bias term found: {bias_ident_param}")
    ...
    def ode_param(t, state):
        x, xdot = state
        xddot = (F_ext(t) - delta_ident_param*xdot - alpha_ident_param*x - beta_ident_param*x**3 - bias_ident_param ) # / m
        return [xdot, xddot]
    def f1_param(x_dot):
        return delta_ident_param * x_dot
    def f2_param(x):
        return alpha_ident_param * x + beta_ident_param * x**3 + bias_ident_param
...
#    plt.show()
    ...
    print(f"delta = {delta_ident_param_STLSQ:.6e}")
    print(f"alpha = {alpha_ident_param_STLSQ:.6e}")
    print(f"beta  = {beta_ident_param_STLSQ:.6e}")
...
#    plt.show()
    ...
    predicted_F1_param = f1_param(xdot_vals).flatten()-f1_param(0).flatten() # parametric-CC
    ...
    predicted_F2_param = f2_param(x_vals).flatten()-f2_param(0).flatten() # parametric-CC
