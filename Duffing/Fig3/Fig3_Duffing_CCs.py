...
for SNR_dB in SNR_dB_list:
    ...
    ax1.plot(time_data, x_data, color='black')
    ax1.set_ylabel("x(t)", fontsize=18) # Increased fontsize to match tick labels
    ax2.plot(time_data, F_ext_noise_data, color='black', linestyle='-')
    ax2.set_xlabel("t", fontsize=18)
    ax2.set_ylabel(r"F$_{ext}$(t)", fontsize=18)
    ...
    custom_ticks(ax2, major_x_interval=10, major_y_interval=0.4, minor_x_interval=5, minor_y_interval=0.2)
    ...
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(5, 8))
    ax1.plot(xdot_vals, f1_fit(xdot_vals)-shift, label="Identified", linewidth=2)
    ax1.plot(xdot_vals, F1(xdot_vals), '--',color='black', label="Theor.", linewidth=2)
    ...
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    ax2.plot(x_vals, f2_fit(x_vals)+shift, label="Identified", linewidth=2)
    ax2.plot(x_vals, F2(x_vals), '--',color='black', label="Theor.", linewidth=3)
    ax2.set_ylabel(r"f$_2$(x)")
    ax2.set_xlabel("x")
    ax2.legend()
...
#    y = predicted_lin_F1_data + bias_correction + correction_extra
    ...
    axs[0].set_ylabel("$f_1(\\dot{x})$", fontsize=24)
    F1_th_range = np.max(F1_th) - np.min(F1_th)
    axs[0].set_ylim(np.min(F1_th) - F1_th_range * 0.5, np.max(F1_th) + F1_th_range * 0.5)
    axs[0].legend(fontsize=15, loc='upper left', bbox_to_anchor=(0, 1),labelspacing=0.05)
    ...
    file_name = "Fig3cd_CC_duffing.pdf"
