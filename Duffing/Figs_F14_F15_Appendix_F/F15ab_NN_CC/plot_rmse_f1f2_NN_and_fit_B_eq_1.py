...
# --- 1. Create a clean X-axis vector for the fit lines ---
# This ensures the line is drawn smoothly across the whole graph, 
# regardless of where your actual data points are.
x_line = np.logspace(0, 3, 100)  # Creates points from 10^0 (1) to 10^3 (1000)
eta_line = x_line / 100.0        # Convert to eta for the formula

# --- Loop 1: Data with Symmetry ---
for col, color, marker, ms in zip(plot1_cols, plot_colors, plot_markers, plot_ms):
    y_data = grouped[col]
    
    # A. Plot Data Points (Scatter)
    ax_bottom.plot(x_bottom, y_data, marker=marker, markersize=ms, 
                   color=color, label=label_map[col], 
                   linewidth=1.5)#, alpha=0.9, zorder=2) # zorder=2 puts points below line
    
    # B. Filter Data for Calculation (Remove zeros/NaNs)
    valid_mask = (y_data > 0) & (eta1 > 0) & (np.isfinite(y_data))
    
    if np.sum(valid_mask) > 1:
        # Perform Fit on VALID data only
        log_y = np.log(y_data[valid_mask])
        log_eta_valid = np.log(eta1[valid_mask])
        
        if FIX_B_TO_ONE:
            B = 1.0
            log_A = np.mean(log_y - log_eta_valid)
            A = np.exp(log_A)
        else:
            coeffs = np.polyfit(log_eta_valid, log_y, 1)
            B = coeffs[0]
            A = np.exp(coeffs[1])
        
        print(f"{label_map[col]:<25} | {A:<15.4e} | {B:<15.4f}")
        
        # C. Plot Fit Line using the SMOOTH x_line
        y_fit_line = A * (eta_line ** B)
        
       # ax_bottom.plot(x_line, y_fit_line, linestyle='--', color=color, 
       #                linewidth=2.5, alpha=0.9, zorder=3) # zorder=3 forces line on top
    else:
        print(f"{label_map[col]:<25} | Skipped (Insufficient valid data)")
...
plt.tight_layout()
plt.savefig("FigF15a_rmse_f1.pdf")
...
for col, color, marker, ms in zip(plot2_cols, plot_colors, plot_markers, plot_ms):
    ...
    # A. Plot Data Points (Scatter)
    ax_bottom.plot(x_bottom, y_data, marker=marker, markersize=ms, 
                   color=color, label=label_map_f2[col], 
                   linewidth=1.5)#, alpha=0.9, zorder=2) # zorder=2 puts points below line
    ...
    if np.sum(valid_mask) > 1:
       ...
       #                linewidth=2.5, alpha=0.9, zorder=3) # zorder=3 forces line on top
        ...
        print(f"{label_map_f2[col]:<25} | Skipped (Insufficient valid data)")
...
ordered_handles = [label_handle_map[l] for l in desired_order if l in label_handle_map]
ordered_labels = [l for l in desired_order if l in label_handle_map]
ax_bottom.legend(ordered_handles, ordered_labels,loc='upper center', labelspacing=0.1, fontsize=fontsize,frameon=False,markerfirst=True,handletextpad=0.05)
# -------------------------------
...
plt.tight_layout()
plt.savefig("FigF15b_rmse_f2.pdf")
plt.show()
