...
col_names = [
    "noise_perc_th", "noise_perc", "SNR_dB", "NN-CC","NN-CC+SR","NN-CC+sym","NN-CC+sym+SR", "Parametric"
]

df = pd.read_csv(filename, skiprows=103, delim_whitespace=True, header=None, names=col_names)
...
label_map = {
    ...
    'NN-CC+sym+SR': r'NN-CC$_{+sym+post\!\!-\!\!SR}$',
    'NN-CC+sym': r'NN-CC$_{+sym}$',
    'NN-CC': 'NN-CC',
    'NN-CC+SR': r'NN-CC$_{+post\!\!-\!\!SR}$',
#    'SINDy_f1': 'SINDy-CC',
#    'LS_f1': 'LS-CC',
#    'SR_f1': 'SR'
}

plot1_cols = ['Parametric','NN-CC+sym+SR','NN-CC+sym','NN-CC+SR','NN-CC']
plot_colors = ['black','blue','teal', 'gray','magenta']#, 'darkgreen', 'darkorange' ]
plot_markers = ['o', 's', '^','D','*']#, 'D', 'p', '*']
plot_ms = [5, 5, 6, 5, 7]#, 5, 6]  # List of marker sizes
...
ax_bottom.legend(fontsize=14,labelspacing=0.2,frameon=False,handletextpad=0.1)
...
ax_bottom.text(0.9, 0.98, '(a)', transform=ax_bottom.transAxes, fontsize=20, verticalalignment='top')



plt.tight_layout()
plt.savefig("Fig8a_time_results.pdf")
plt.show()
