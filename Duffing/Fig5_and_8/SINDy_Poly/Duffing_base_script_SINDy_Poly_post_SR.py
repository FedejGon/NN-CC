...
#SNR_dB_list = list(np.linspace(10, -20, 31 ))  # ∞, 20, 17.5, ..., -5

#SNR_dB_list = list(np.linspace(28, -20, 49 ))  # ∞, 20, 17.5, ..., -5
...
# Validation Loop Variables
    ...
    while valid_trials < n_trials:
        ...
        #############################################################        
        ################ SINDY k0 u ###### validation of the model
        print("Simulating Sindy with k0 u")
        x_simulated_Sindy_ku0 = []
        x_dot_simulated_Sindy_ku0 = []
        start = time.time()
        try:
        ...
        try:
            ...
            if np.any(np.isnan(x_simulated_Sindy)) or np.any(np.isinf(x_simulated_Sindy)):
                print(f"Skipping trial {valid_trials}: SINDy-CC simulation returned NaNs or infs.")
        except Exception as e:
            print(f"Skipping trial {valid_trials}: Exception during SINDy-CC simulation -> {e}")
        end = time.time()  
        elapsed = end - start
        print(f"Solve_ivp finished in {elapsed:.3f} seconds")

        ################ SINDy-CC+sym ###### validation of the model
        print("Simulating Sindy-CC+sym restrictions")
        x_simulated_Sindy_sym = []
        x_dot_simulated_Sindy_sym = []
        start = time.time()
        try:
            sol = solve_ivp(fitted_model_sindy_sym, t_span_val, y0_val, t_eval=t_val,method='LSODA')
            x_simulated_Sindy_sym = sol.y[0]       # Posición
            x_dot_simulated_Sindy_sym = sol.y[1]   # Velocidad
            # check for NaNs or infs just in case
            if np.any(np.isnan(x_simulated_Sindy_sym)) or np.any(np.isinf(x_simulated_Sindy_sym)):
                print(f"Skipping trial {valid_trials}: SINDy-CC+sym simulation returned NaNs or infs.")
        except Exception as e:
            print(f"Skipping trial {valid_trials}: Exception during SINDy-CC+sym simulation -> {e}")
        end = time.time()  
        elapsed = end - start
        print(f"Solve_ivp finished in {elapsed:.3f} seconds")

        ################ SINDy-CC+sym+SR ###### validation of the model
        print("Simulating Sindy-CC+sym+SR restrictions")
        x_simulated_Sindy_sym_SR = []
        x_dot_simulated_Sindy_sym_SR = []
        start = time.time()
        try:
            sol = solve_ivp(fitted_model_sindy_sym_SR, t_span_val, y0_val, t_eval=t_val,method='LSODA')
            x_simulated_Sindy_sym_SR = sol.y[0]       # Posición
            x_dot_simulated_Sindy_sym_SR = sol.y[1]   # Velocidad
            # check for NaNs or infs just in case
            if np.any(np.isnan(x_simulated_Sindy_sym_SR)) or np.any(np.isinf(x_simulated_Sindy_sym_SR)):
                print(f"Skipping trial {valid_trials}: SINDy-CC+sym+SR simulation returned NaNs or infs.")
        except Exception as e:
            print(f"Skipping trial {valid_trials}: Exception during SINDy-CC+sym+SR simulation -> {e}")
        end = time.time()  
        elapsed = end - start
        print(f"Solve_ivp finished in {elapsed:.3f} seconds")
...
#        plt.plot(t_val[0:-1], x_val_sindy[:,1],"--",color='darkgreen', label="$\\dot{x}_{val} Sindy$",linewidth='3')
        ...
        axs[0, 0].set_ylim(np.min(x_simulated_th)-0.2, np.max(x_simulated_th)+0.2)
        ...
        axs[0, 1].set_ylim(np.min(x_dot_simulated_th)-0.2, np.max(x_dot_simulated_th)+0.2)
...
#                time_chaos_x_parametric_list.append(t_val[i])
        ...
        time_chaos_x_Sindy_sym=t_val[len(x_simulated_Sindy_sym)-1]
        for i in range(len(x_simulated_Sindy_sym)):
            diff = abs(x_simulated_Sindy_sym[i] - x_simulated_th[i])
            if diff > threshold_chaos:
                #time_chaos_x_Sindy_list.append(t_val[i])
                time_chaos_x_Sindy_sym=t_val[i]
                break
        time_chaos_x_Sindy_sym_list.append(time_chaos_x_Sindy_sym)
        #time_chaos_x_Sindy_ku0_list.append(t_val[-1])

        time_chaos_x_Sindy_sym_SR=t_val[len(x_simulated_Sindy_sym_SR)-1]
        for i in range(len(x_simulated_Sindy_sym_SR)):
            diff = abs(x_simulated_Sindy_sym_SR[i] - x_simulated_th[i])
            if diff > threshold_chaos:
                #time_chaos_x_Sindy_list.append(t_val[i])
                time_chaos_x_Sindy_sym_SR=t_val[i]
                break
        time_chaos_x_Sindy_sym_SR_list.append(time_chaos_x_Sindy_sym_SR)
        #time_chaos_x_Sindy_ku0_list.append(t_val[-1])
        ...
        time_matrix_append = np.column_stack([
            ...
            time_chaos_x_Sindy_sym,
            time_chaos_x_Sindy_sym_SR,
            time_chaos_x_LS,
            #time_chaos_x_SR,
        ...
        with open(file_path, 'a') as f:
            np.savetxt(f, time_matrix_append,
                       ...
                       header="nois_th nois nois_db  Sindy-CC Sindy_ku0  Sindy-CC+sym Sindy-CC+sym+SR  Poly-CC  Parametric" if not file_exists else '',
