...
for SNR_dB in SNR_dB_list:
    ...
    try:
        sol = solve_ivp(ode_sr, t_span, y0, t_eval=t_simul,method='LSODA', rtol=1e-9, atol=1e-12)
        if np.any(np.isnan(x_simulated_LS)) or np.any(np.isinf(x_simulated_LS)):
            print(f"Skipping trial {i+1}: LS simulation returned NaNs or infs.")
            # continue your RMSE calculations and plots here...
    except Exception as e:
...
#
        ...
        print("Simulating Sindy-CC")
        x_simulated_Sindy = []
        x_dot_simulated_Sindy = []
        start = time.time()
        
        try:
            sol_sindy = solve_ivp(fitted_model_sindy, t_span, y0, t_eval=t_simul,method='LSODA', rtol=1e-9, atol=1e-12)
            #x_simulated_LS = sol_fit.y[0]       # Posición
            #x_dot_simulated_LS = sol_fit.y[1]   # Velocidad
            x_simulated_Sindy = sol_sindy.y[0]
            x_dot_simulated_Sindy = sol_sindy.y[1]
            # check for NaNs or infs just in case
            if np.any(np.isnan(x_simulated_LS)) or np.any(np.isinf(x_simulated_LS)):
                print(f"Skipping trial {i+1}: SINDy simulation returned NaNs or infs.")
                #  continue your RMSE calculations and plots here...
...
#        plt.plot(t_val[0:-1], x_val_sindy[:,1],"--",color='darkgreen', label="$\\dot{x}_{val} Sindy$",linewidth='3')
        ...
        axs[0, 0].plot(t_val, x_simulated_NN_nosym, "--", color='violet', label="NN-CC retrained", linewidth=3)
        ...
        axs[1, 1].plot(t_val[0:len(x_dot_simulated_Sindy)], x_dot_simulated_Sindy - x_dot_simulated_th[0:len(x_dot_simulated_Sindy)], '--', color='darkgreen', label="SINDy-CC", linewidth=3)
...
#                time_chaos_x_parametric_list.append(t_val[i])
        ...
        with open(file_path, 'a') as f:
            np.savetxt(f, time_matrix_append,
                       header="nois_th nois nois_db  NN_nosym   NN_sym NN_sym_SR  Sindy-CC  Poly  SR  Parametric" if not file_exists else '',
     ...
     #   print("LS results")
        ...
        with open(file_path, 'a') as f:
            np.savetxt(f, rmse_matrix_append,
                       header="noise_th noise noise_db rmse_x_NN rmse_x_dot_NN rmse_x_Sindy rmse_x_dot_Sindy rmse_x_LS rmse_x_dot_LS rmse_x_NN_nosym rmse_x_dot_NN_nosym rmse_x_NN_SR rmse_x_dot_NN_SR" if not file_exists else '',
    ...
    with open(file_path, 'a') as f:
        np.savetxt(f, rmse_matrix,
                   header="noise_th noise_meas noise_db rmse_x_NN rmse_x_dot_NN rmse_x_Sindy rmse_x_dot_Sindy rmse_x_LS rmse_x_dot_LS rmse_x_NN_nosym rmse_x_dot_NN_nosym rmse_x_NN_SR rmse_x_dot_NN_SR" if not file_exists else '',
                   fmt="%.8f", delimiter=" ", comments='')
