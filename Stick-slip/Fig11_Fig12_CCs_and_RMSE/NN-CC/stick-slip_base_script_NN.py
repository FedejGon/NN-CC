...
#    y = predicted_lin_F1_data + bias_correction + correction_extra
    model_f1SR_nosym = PySRRegressor(
        niterations=100,
        binary_operators=["+", "-", "*"],
        unary_operators=["tanh"],#"sin", "cos", "exp", "log", "abs", "sqrt","sign"],
    ...
    model_f2SR_nosym = PySRRegressor(
        niterations=100,
        binary_operators=["+", "-", "*"],
        unary_operators=["tanh"],# log", "abs", "sqrt","sin", "cos", "exp"],
        loss="loss(x, y) = (x - y)^2",
        maxsize=20,
        populations=20,
        #verbosity=1,
    ...
    xdot_sim = sol.y[1]
    plt.figure(figsize=(8,5))
    plt.plot(time_data, x_sim, label=r"NN-CC$_{+post\!\!-\!\!SR}$", linewidth=2)
    plt.plot(time_data, x_data,"--",color='black', label="Training data", linewidth=2)
    plt.xlabel("t")
    plt.ylabel("x(t)")
    plt.legend()
...
#    y = predicted_lin_F1_data + bias_correction + correction_extra
    model_f1SR = PySRRegressor(
        niterations=100,
        binary_operators=["+", "-", "*"],
        unary_operators=["tanh"], # "cos", "exp", "log", "abs", "sqrt","sign"],
        #unary_operators=["abs", "sqrt","sign"],
        loss="loss(x, y) = (x - y)^2",
        maxsize=20,
        populations=20,
...
# Validation Loop Variables
    ...
    while valid_trials < n_trials:
        ...
        if len(x_simulated_NN_SR_nosym) != len(t_val):
            print("Warning:")
            print("NN-CC+SR nosym finished integration before maximum simulation time")
            print("time:",t_val[len(x_simulated_NN_SR_nosym)-1])
        if len(x_simulated_NN) != len(t_val):
            print("Warning:")
            print("NN-CC+sym finished integration before maximum simulation time")
            print("time:",t_val[len(x_simulated_NN)-1])
        if len(x_simulated_NN_SR) != len(t_val):
            print("Warning:")
            print("NN-CC+sym+SR finished integration before maximum simulation time")
...
#                time_chaos_x_parametric_list.append(t_val[i])
        ...
        with open(file_path, 'a') as f:
            np.savetxt(f, time_matrix_append,
                       ...
                       header="nois_th nois nois_db  NN_nosym NN_nosym+SR NN+sym NN+sym+SR  Parametric" if not file_exists else '',
    ...
    #    print("parametric results")
        ...
        print(f"NN results nosym \t - x: {rmse_x_NN_nosym:.6f}, x': {rmse_x_dot_NN_nosym:.6f}")
        rmse_x_NN_nosym_list.append(rmse_x_NN_nosym)
        rmse_x_dot_NN_nosym_list.append(rmse_x_dot_NN_nosym)

        rmse_x_NN_SR_nosym = np.sqrt(np.mean((x_simulated_th[0:len(x_simulated_NN_SR_nosym)] - x_simulated_NN_SR_nosym) ** 2))
        rmse_x_dot_NN_SR_nosym = np.sqrt(np.mean((x_dot_simulated_th[0:len(x_dot_simulated_NN_SR_nosym)] - x_dot_simulated_NN_nosym) ** 2))
        print(f"NN_SR results nosym \t - x: {rmse_x_NN_SR_nosym:.6f}, x': {rmse_x_dot_NN_SR_nosym:.6f}")
        rmse_x_NN_SR_nosym_list.append(rmse_x_NN_SR_nosym)
        rmse_x_dot_NN_SR_nosym_list.append(rmse_x_dot_NN_SR_nosym)
...
#        rmse_x_dot_LS_list.append(rmse_x_dot_LS)
        ...
        with open(file_path, 'a') as f:
            np.savetxt(f, rmse_matrix_append,
                       header="noise_th noise noise_db xNN vNN  xNN+SR vNN+SR xNN+sym vNN+sym xNN+sym+SR vNN+sym+SR xparam vparam" if not file_exists else '',
