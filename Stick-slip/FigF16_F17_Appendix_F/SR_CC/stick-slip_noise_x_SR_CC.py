...
#        plt.close()
    ...
    total_target = F_ext(time_data) - x_ddot_data
...
#    predicted_F1_NNCC_sym_SR = model_f1SR.predict(xdot_vals.reshape(-1,1)) # NN-CC(+sym+post-SR)
    predicted_F1_SR_CC = safe_f1(xdot_vals).flatten() # SR-CC
...
#    predicted_F2_NNCC_sym_SR = model_f2SR.predict(x_vals.reshape(-1,1)) # NN-CC(+sym+post-SR)
    predicted_F2_SR_CC = safe_f2(x_vals).flatten() # SR-CC
...
#
#    n_trials = 0  # number of random initial conditions
#    #rmse_x_NN_list = []
#    #rmse_x_dot_NN_list = []
#    #rmse_x_Sindy_list = []
#    #rmse_x_dot_Sindy_list = []
...
#    n_trials = 0        # Target number of valid simulations
