...
col_names = [
    "noise_perc_th", "noise_perc", "SNR_dB", "Param_f1","Param_f2","SR_f1","SR_f2","SR_CC_f1","SR_CC_f2"
]
...
try:
    df = pd.read_csv(filename, skiprows=8, delim_whitespace=True, header=None, names=col_names)
...
# PLOT 1: F1
# =========================================================
plot1_cols = ['Param_f1', 'SR_f1', 'SR_CC_f1']
...
for col, color, marker, ms in zip(plot1_cols, plot_colors, plot_markers, plot_ms):
    ...
    ax_bottom.plot(x_bottom, y_data, marker=marker, markersize=ms, 
                   color=color, label=label_map_f1[col], linewidth=2.0)#, alpha=0.8)
    ...
    if col == 'SR_f1':
        ...
        x_fit_sr = np.linspace(x_bottom.min(), 1000, 50) 
...
ax_bottom.text(0.1, 0.98, '(e)', transform=ax_bottom.transAxes, fontsize=20, verticalalignment='top')
...
plt.tight_layout()
plt.savefig("FigF17e_rmse_f1.pdf")


# =========================================================
# PLOT 2: F2
# =========================================================
plot2_cols = ['Param_f2',  'SR_f2', 'SR_CC_f2']
...
for col, color, marker, ms in zip(plot2_cols, plot_colors, plot_markers, plot_ms):
    ...
    ax_bottom.plot(x_bottom, y_data, marker=marker, markersize=ms, 
                   color=color, label=label_map_f2[col], linewidth=2)#, alpha=0.8)
...
ax_bottom.text(0.1, 0.98, '(f)', transform=ax_bottom.transAxes, fontsize=20, verticalalignment='top')
...
plt.tight_layout()
plt.savefig("FigF17f_rmse_f2.pdf")
plt.show()
