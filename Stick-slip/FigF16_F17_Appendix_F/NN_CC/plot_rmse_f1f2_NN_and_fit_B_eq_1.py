...
for col, color, marker, ms in zip(plot1_cols, plot_colors, plot_markers, plot_ms):
    ...
    ax_bottom.plot(x_bottom, y_data, marker=marker, markersize=ms, 
                   color=color, label=label_map[col], linewidth=1.5)#, alpha=0.9)
...
plt.tight_layout()
plt.savefig("FigF17a_rmse_f1.pdf")
...
for col, color, marker, ms in zip(plot2_cols, plot_colors, plot_markers, plot_ms):
    ...
    # Plot points
    ax_bottom.plot(x_bottom, y_data, marker=marker, markersize=ms, 
                   color=color, label=label_map_f2[col], linewidth=1.5)#, alpha=0.9)
...
ordered_handles = [label_handle_map[l] for l in desired_order if l in label_handle_map]
ordered_labels = [l for l in desired_order if l in label_handle_map]
ax_bottom.legend(ordered_handles, ordered_labels,loc='upper center', labelspacing=0.1, fontsize=fontsize,frameon=False,handletextpad=0.1) #markerfirst=False
# -------------------------------
...
plt.tight_layout()
plt.savefig("FigF17b_rmse_f2.pdf")
plt.show()
