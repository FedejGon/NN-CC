...
# Plot Differentiation Results
    ...
    # Label (a) inside plot
    ax[0].text(0.02, 0.1, '(a)', transform=ax[0].transAxes, fontsize=24 ) #, weight='bold')
    ...
    # Label (b) inside plot
    ax[1].text(0.02, 0.1, '(b)', transform=ax[1].transAxes, fontsize=24 ) #, weight='bold')
    # Custom ticks: X(Major 10, Minor 2), Y(Major 1, Minor 0.5)
    custom_ticks(ax[1], 10, 0.5 ,2.0, 0.25)
    ...
    # Label (c) inside plot
    ax[2].text(0.02, 0.1, '(c)', transform=ax[2].transAxes, fontsize=24) #, weight='bold')
    # Custom ticks: X(Major 10, Minor 2), Y(Major 5, Minor 1)
    custom_ticks(ax[2], 10, 1.0, 2.0, 0.5)

    plt.tight_layout()
    plt.savefig("FigF16_SG_filter.pdf")
    plt.show()
